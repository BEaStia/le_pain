...
module LePain
  module TaskStores
    class PostgresStore < Base
      def initialize(connection_string: nil, connection: nil, pool_size: 5, ttl: 86400)
        require 'pg'
      ...
      def create(task)
        with_connection do |conn|
          conn.exec_params(
            <<~SQL,
              INSERT INTO lepain_tasks (id, type, state, payload, result, error, context, created_at, updated_at, started_at, completed_at)
      ...
      def update(id, &block)
        with_connection do |conn|
          result = conn.exec_params('SELECT * FROM lepain_tasks WHERE id = $1', [id])
          return nil if result.ntuples.zero?
      ...
      def list(limit: 50, state: nil, type: nil)
        with_connection do |conn|
          ...
          result = conn.exec_params(
            "SELECT * FROM lepain_tasks #{where_clause} ORDER BY created_at DESC LIMIT $#{param_idx}",
            params
          )
